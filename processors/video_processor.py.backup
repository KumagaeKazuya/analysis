...
class VideoProcessor:
    ...
    def run_detection_tracking(self, frame_dir: Path, video_name: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # タイル推論が有効な場合
            if self.tile_enabled:
                return self._run_tile_inference(frame_dir, result_dir, video_name, processing_config)
            else:
                return self._run_normal_inference(frame_dir, result_dir, video_name, processing_config)
    ...
    def _run_tile_inference(self, frame_dir: Path, result_dir: Path, video_name: str, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" not in result:
                ...
                # タイル推論統計をログ出力
                if "processing_stats" in result and "tile_stats" in result["processing_stats"]:
                    tile_stats = result["processing_stats"]["tile_stats"]
                    if tile_stats:
                        self.logger.info(f"📊 タイル統計 - 平均タイル数/フレーム: {tile_stats.get('avg_tiles_per_frame', 0):.1f}")
                        self.logger.info(f"📊 タイル統計 - NMS削減率: {tile_stats.get('avg_nms_reduction', 0):.1%}")

            return result

        except ImportError:
            self.logger.error("タイル推論関数が見つかりません")
            return {"error": "tile_inference_function_not_found", "success": False}
        except Exception as e:
    ...
    def _run_normal_inference(self, frame_dir: Path, result_dir: Path, video_name: str, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            result = analyze_frames_with_tracking(
                str(frame_dir),
                str(result_dir),
