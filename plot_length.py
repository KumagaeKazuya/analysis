...
def plot_detection_histogram(df, frame_col, pid, interval, out_dir):
    ...
    bins = np.arange(0, frame_max + interval, interval)
    counts, edges = np.histogram(frames, bins=bins)
    plt.figure(figsize=(8, 4))
    plt.bar(range(len(counts)), counts, tick_label=[f"{edges[i]:.0f}-{edges[i+1]-1:.0f}" for i in range(len(counts))])
    plt.xlabel("フレーム区間")
    plt.ylabel("検出数")
    plt.title(f"person_id={pid} の{interval}フレーム毎の検出数")
    plt.xticks(rotation=45, ha='right')
    plt.gca().yaxis.set_major_locator(plt.MaxNLocator(integer=True))  # 縦軸を整数に
    plt.tight_layout()
...
def main():
    ...
    # --- summary.csvから3割以上person_idのみgraph_frames_all.csvからヒストグラムをhistサブフォルダに出力 ---
    summary_df['rate'] = summary_df['frames_with_id'] / summary_df['total_frames']
    valid_ids = summary_df.loc[summary_df['rate'] >= 0.3, 'person_id'].astype(int).tolist()
    print(f"3割以上検出person_id: {valid_ids}")
    ...
    if graph_csv_file and os.path.exists(graph_csv_file):
        ...
        print(f"graph_frames_all.csvから3割以上person_idのヒストグラムを {subfolders['hist']} 以下に保存しました。")
    else:
